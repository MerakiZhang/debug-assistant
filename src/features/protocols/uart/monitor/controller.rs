...
use crate::features::protocols::uart::monitor::state::{ConfigField, Focus, SerialMonitorState};
use crate::features::protocols::uart::monitor::Action;
...
pub fn handle_key(
    ...
    match state.focus {
        ...
        Focus::Receive => handle_receive_key(state, code),
        Focus::HelpOverlay => {}
    }
    Ok(Action::None)
}
...
fn handle_setup_key(
    ...
    match (code, mods) {
        ...
        (KeyCode::Char('r'), KeyModifiers::NONE) if state.config_field == ConfigField::PortName => {
...
fn handle_receive_key(state: &mut SerialMonitorState, code: KeyCode) {
