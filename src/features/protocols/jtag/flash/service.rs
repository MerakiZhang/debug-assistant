...
use crate::features::protocols::flashing::state::FlasherState;

pub fn start_flash(state: &mut FlasherState, tx: mpsc::Sender<AppEvent>) -> anyhow::Result<()> {
    let chip_name = state.jtag.chip_name.trim().to_string();
    if chip_name.is_empty() {
        bail!("Chip name is empty (example: STM32F103C8)");
    }

    let file_path = state.jtag.file_path.trim().to_string();
    ...
    let bin_base_address = if file_kind == flashing::FormatKind::Bin {
        Some(parse_address(&state.jtag.bin_base_address)?)
    } else {
        None
    };
    let format = format_for_download(&file_path, bin_base_address)?;

    let probe_idx = state.jtag.probe_idx;
    let speed_khz = state.jtag_speed_khz();
    let verify = state.jtag.verify;
    let reset_run = state.jtag.reset_run;
    let stop_flag = Arc::new(AtomicBool::new(false));
    state.stop_flag = Some(stop_flag.clone());

    std::thread::spawn(move || {
        let result = run_jtag(
            probe_idx, &chip_name, &file_path, format, speed_khz, verify, reset_run, &tx,
            &stop_flag,
        );
        match result {
...
fn run_jtag(
    ...
    probe
        ...
        .context("Failed to switch probe to JTAG mode")?;
    let actual_speed = probe
        .set_speed(speed_khz)
        .context("Failed to configure JTAG speed")?;
    log!("JTAG speed set to {} kHz", actual_speed);

    check_stop!();
    log!("Attaching to target {} over JTAG...", chip_name);
