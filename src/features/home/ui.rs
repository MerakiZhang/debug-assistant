...
fn render_header(frame: &mut Frame, area: Rect) {
    ...
    let header = Paragraph::new(vec![
        ...
        Line::from(" UART, JTAG and SWD workflows are grouped by physical/debug protocol."),
    ])
...
fn render_protocol_nav(frame: &mut Frame, state: &HomeState, area: Rect) {
    ...
    let mut constraints = vec![Constraint::Length(3); MENU_ITEMS.len()];
    ...
    for (idx, item) in MENU_ITEMS.iter().enumerate() {
        ...
        let status = match *item {
            ...
            "I2C" => "Tool lane planned",
            "SPI" => "Tool lane planned",
            _ => "Leave console",
        };
        frame.render_widget(
            Paragraph::new(vec![
...
fn render_protocol_detail(frame: &mut Frame, selected: usize, area: Rect) {
    let (title, color, lines): (&str, Color, &[&str]) = match selected {
        0 => (
            ...
            theme::SUCCESS,
            &[
                "RX/TX serial terminal with ASCII, HEX and BOTH display modes",
        ...
        3 => (
            ...
            Color::Blue,
            &[
                "Planned I2C bus scan workflow",
                "Planned register read/write workflow",
                "Transport adapter not selected yet",
                "No hardware access is initialized from this placeholder",
            ],
        ),
        4 => (
            " SPI Tool Bay ",
            Color::LightBlue,
            &[
                "Planned raw transfer workflow",
                "Planned SPI flash workflow",
                "Transport adapter not selected yet",
                "No hardware access is initialized from this placeholder",
            ],
        ),
