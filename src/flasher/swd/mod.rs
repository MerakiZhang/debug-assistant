...
pub fn start_flash(state: &mut FlasherState, tx: mpsc::Sender<AppEvent>) -> anyhow::Result<()> {
    ...
    let file_path = state.swd_file_path.trim().to_string();
    ...
    let bin_base_address = if file_kind == flashing::FormatKind::Bin {
        Some(parse_address(&state.swd_bin_base_address)?)
    } else {
        None
    };
    ...
    let probe_idx = state.swd_probe_idx;
    let speed_khz = state.swd_speed_khz();
    let connect_mode = state.swd_connect_mode;
    let verify = state.swd_verify;
    let reset_run = state.swd_reset_run;
