...
use crate::serial::{
    SerialConfig, BAUD_PRESETS, DATA_BITS_OPTIONS, FLOW_CONTROL_OPTIONS,
...
impl App {
    ...
    pub fn push_rx(&mut self, data: Vec<u8>) {
        ...
        // Flush every complete line (terminated by \n)
        ...
        // Safety flush: prevent unbounded buffering for binary/no-newline protocols
    ...
    /// Flush whatever is still in the line buffer after a 100 ms silence.
    /// Call this on every Tick event.
    ...
    fn commit_rx_line(&mut self, mut raw: Vec<u8>) {
        // Strip trailing CR / LF so the log stays clean
    ...
    pub fn open_config_popup(&mut self) {
        ...
        self.config_port_idx = self.config_port_list
            .iter()
    ...
    pub fn config_field_next_option(&mut self) {
        match self.config_field {
            ConfigField::PortName    => self.config_port_idx    = (self.config_port_idx + 1)    % self.config_port_list.len().max(1),
            ConfigField::BaudRate    => self.config_baud_idx    = (self.config_baud_idx + 1)    % BAUD_PRESETS.len(),
            ConfigField::DataBits    => self.config_databits_idx= (self.config_databits_idx + 1)% DATA_BITS_OPTIONS.len(),
            ConfigField::StopBits    => self.config_stopbits_idx= (self.config_stopbits_idx + 1)% STOP_BITS_OPTIONS.len(),
            ConfigField::Parity      => self.config_parity_idx  = (self.config_parity_idx + 1)  % PARITY_OPTIONS.len(),
            ConfigField::FlowControl => self.config_flow_idx    = (self.config_flow_idx + 1)    % FLOW_CONTROL_OPTIONS.len(),
        }
    }

    pub fn config_field_prev_option(&mut self) {
        fn wrap_dec(val: usize, len: usize) -> usize {
            if val == 0 { len.saturating_sub(1) } else { val - 1 }
        }
        match self.config_field {
            ConfigField::PortName    => self.config_port_idx     = wrap_dec(self.config_port_idx,     self.config_port_list.len().max(1)),
            ConfigField::BaudRate    => self.config_baud_idx     = wrap_dec(self.config_baud_idx,     BAUD_PRESETS.len()),
            ConfigField::DataBits    => self.config_databits_idx = wrap_dec(self.config_databits_idx, DATA_BITS_OPTIONS.len()),
            ConfigField::StopBits    => self.config_stopbits_idx = wrap_dec(self.config_stopbits_idx, STOP_BITS_OPTIONS.len()),
            ConfigField::Parity      => self.config_parity_idx   = wrap_dec(self.config_parity_idx,   PARITY_OPTIONS.len()),
            ConfigField::FlowControl => self.config_flow_idx     = wrap_dec(self.config_flow_idx,     FLOW_CONTROL_OPTIONS.len()),
        }
    }
    ...
    s.split_whitespace()
        .map(|tok| {
            u8::from_str_radix(tok, 16)
                .map_err(|_| anyhow::anyhow!("Invalid hex token: '{}'", tok))
        })
        .collect()
}
